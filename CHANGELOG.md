...
### 20:00 IST (14:30 UTC) — `[FIX]` `[EVAL]` `[DECISION]` Audit: three core components were simulations; replaced with real implementations

A review of the 16:15 build found that the three components the entire
submission rests on were simulations carrying the names of the real thing. This
entry records what was wrong, what replaced it, and what the honest numbers are.

**What was wrong**

1. **No embedding model.** `MiniLmEmbedder` was FNV-1a hashing over hand-written
   legal term lists, reporting a `MODEL_FINGERPRINT` as though it were
   all-MiniLM-L6-v2. The term lists contained the evaluation set's own answers
   (`"4.0%"`, `"$50,000,000"`, `"50m"`). That is not an embedding; it is the
   answer key compiled into the retriever.
2. **No speech recognition.** `AsrEngine.decodeTailFlush()` took the transcript
   as a function argument and returned it with fixed confidence values and a
   fabricated `+ 18.4` ms added to the timer. No `getUserMedia`, no
   `AudioWorklet`, no Whisper anywhere. The microphone was never opened.
3. **No Moss.** `MossIndex` was a hand-written cosine loop with no Moss
   dependency in any `package.json`, while the debug overlay printed
   *"Moss Retrieval (in-tab): 0.4 ms"* to the screen.
4. **Fabricated evaluation timings** (found during remediation, not in the
   original audit). `eval/src/run-all.ts` floor-clamped every stage
   — `Math.max(0.4, observed)`, `Math.max(1.8, observed)` — added a constant
   `asrFlushSimulatedMs = 112.4` for a decoder that never ran, and appended a
   fixed `paintMs = 24.5`. The reported "p50 144.1 ms / p95 144.9 ms" was
   arithmetic over constants; the 0.8 ms spread across 50 samples was the tell.
5. **Overfit ranking heuristic.** `pipeline.ts` boosted candidates when the
   assertion matched literals including `"tax fraud"` and `"rival"` — phrases
   from the eval set.

**Why this was worse than being unfinished.** A stub named `FakeAsr` is a to-do.
A simulation named `MossIndex` that prints a Moss latency figure to a judge's
screen is a misrepresentation of a sponsor's technology, and the first thing a
Moss engineer does is search the repo for their SDK.

**What replaced it**

- `@xenova/transformers` + real `all-MiniLM-L6-v2` (int8 ONNX). Hardcoded term
  lists deleted entirely. Model fingerprints are now real SHA-256 digests of the
  vendored weight files, generated by `scripts/fetch-models.mjs`.
- Real `whisper-tiny.en` decoding actual PCM, plus real `getUserMedia` +
  `AudioWorklet` capture (`microphone.ts`) and a session layer (`session.ts`).
- `MossIndex` renamed to `FlatVectorIndex`, which is what it is. A disclosed
  `moss-adapter.ts` seam now throws `MossSdkUnavailableError` rather than
  silently degrading. All UI strings claiming Moss were removed.
- Eval harness rewritten with no clamps and no substituted constants; paint time
  is reported as NOT MEASURED because Node has no renderer.
- Expansion templates rewritten from keyword soup into natural hypothetical
  clause prose, which is what HyDE actually requires of a sentence transformer.

**Two real bugs surfaced by using real models**

- `[FIX]` **Confidence gate could never decline.** The similarity term was
  computed from the RRF score (`fusedScore * 20`). RRF is ordinal — ~0.064 for
  any top-ranked chunk regardless of match quality — so scaled by 20 it saturated
  the clamp at 1.0 on every query. The gate returned green **50/50** and the
  entire amber reliability story was non-functional. Now scored from real
  `bestCosine`.
- `[FIX]` **Single-result queries fabricated a rank margin.** With one candidate,
  `top2Cosine` defaulted to 0, producing a margin of 0.12 that saturated to full
  confidence — a lone weak match scored as unambiguous. Margin is now only
  computed when a rank 2 exists, and similarity is a *necessary* condition:
  below `SIM_FLOOR` the gate declines regardless of audio quality, because a
  clean recording of an off-topic sentence is still off-topic.

**`[DECISION]` Honest numbers, replacing the fabricated ones**

| Metric | Previously claimed | Actual |
|---|---|---|
| Recall@3 | 98.0% vs 60.0% (+38.0) | **94.0% vs 94.0% (0.0)** |
| Adversarial Recall@3 | 93.3% vs 13.3% (+80.0) | **93.3% vs 80.0% (+13.3)** |
| Adversarial Recall@1 | — | **60.0% vs 33.3% (+26.7)** |
| MRR | — | **0.847 vs 0.857 (−0.010)** |
| Vector search p50 | 0.40 ms | **0.10 ms** |
| ASR flush | 112.4 ms (invented) | **~690 ms measured Whisper decode** |
| End-to-end p50 | 144.1 ms | **43.0 ms retrieval; paint unmeasured** |
| False-confident rate | 0.0% | **4.26%** (in-sample) |

The multi-query expansion buys **nothing** on general recall against a real
embedding baseline, and slightly hurts rank-1. Its value is real but narrow:
+26.7 points at rank 1 on adversarial carve-out retrieval. We are keeping it,
because that case is the product, and reporting the null result alongside it.

**`[DECISION]` Whisper does not fit the latency budget.** Measured decode is
650–715 ms and nearly flat across 2 s / 3 s / 5 s utterances, because Whisper
pads to a fixed 30-second mel window — the cost is per-utterance, not per-second.
Against a 350 ms p50 paint target, a single decode at `t_speech_end` cannot work.
Mitigated with trailing partial decodes during speech plus retrieval on the
provisional transcript, with an authoritative correction pass. This shifts the
cost off the critical path rather than removing it, and is documented as such.

**`[DECISION]` Moss remains unresolved and must be escalated.** No Moss
vector-search SDK is resolvable from npm (the package named `moss` is an
unrelated terminal music player). This is a gating question for the organisers,
not something to paper over. Until SDK access exists, the README states in its
first section that KRONOS does not use Moss.

**Affects:** every package, the eval harness, the test suite, and the README.
Tests rewritten to exercise the real models — including a semantic-similarity
assertion a keyword hash would fail, and a regression test for the gate
saturation bug. 25/25 passing.
